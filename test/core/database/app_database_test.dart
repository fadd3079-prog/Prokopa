...
void main() {
  ...
  Future<String> temporaryDatabasePath() async {
    final directory = await Directory.systemTemp.createTemp('prokopa_database_');
  ...
  test('fresh database contains only the migration ledger', () async {
  ...
  test('initial schema version is explicit and stored in SQLite', () async {
    ...
    expect(databaseSchemaVersion, 1);
  ...
  test('an existing version-zero database upgrades to the current schema', () async {
    ...
    await previous.execute('CREATE TABLE preservation_probe (id INTEGER PRIMARY KEY)');
    ...
    expect(await upgraded.query('schema_migrations'), hasLength(1));
  });
  ...
  test('fresh migration sequence runs in ascending dependency order', () async {
    ...
    expect(
      await database.query('schema_migrations', columns: ['version'], orderBy: 'version'),
      [
        {'version': 1},
        {'version': 2},
        {'version': 3},
  ...
  test('older version upgrades only pending migrations and preserves rows', () async {
    final databasePath = await temporaryDatabasePath();
    final previous = await openMigrationFixture(databasePath, testMigrations.take(2).toList());
    await previous.insert('migration_probe', {'id': 7});
    final priorLedger = await previous.query('schema_migrations', orderBy: 'version');
    await previous.close();
    ...
    expect(
      await upgraded.query('schema_migrations', where: 'version <= 2', orderBy: 'version'),
      priorLedger,
    );
    expect(await upgraded.query('schema_migrations'), hasLength(3));
  });
  ...
  test('out-of-order migrations are rejected before changing schema', () async {
    ...
    await expectLater(
      database.transaction((transaction) => runDatabaseMigrations(
        transaction,
        1,
        3,
        [testMigrations[2], testMigrations[1]],
      )),
      throwsStateError,
    );
  ...
  test('a missing migration is rejected before changing schema', () async {
    ...
    expect(await database.query('schema_migrations'), hasLength(1));
    expect(
      await database.rawQuery("SELECT name FROM sqlite_master WHERE name = 'migration_probe'"),
      isEmpty,
    );
  });

  test('failed upgrade rolls back every pending migration and the version', () async {
    final databasePath = await temporaryDatabasePath();
    ...
    await expectLater(
      ...
      throwsA(isA<DatabaseMigrationException>()
        .having((error) => error.version, 'version', 3)
        .having((error) => error.name, 'name', 'failing_migration')
        .having((error) => error.cause, 'cause', isA<DatabaseException>())),
    );
    ...
    expect(
      await reopened.rawQuery("SELECT name FROM sqlite_master WHERE name = 'migration_probe'"),
      isEmpty,
    );
  });

  test('failed initial migration leaves version zero and no partial schema', () async {
    final databasePath = await temporaryDatabasePath();
    ...
    expect(
      await raw.rawQuery("SELECT name FROM sqlite_master WHERE name = 'schema_migrations'"),
      isEmpty,
    );
    await raw.close();
  ...
  test('opening a newer schema refuses downgrade and preserves its data', () async {
    ...
    final reopened = await openMigrationFixture(databasePath, testMigrations);
  ...
  test('SQL failure rolls back earlier transaction writes', () async {
    ...
    expect(await database.query('transaction_probe'), [{'id': 2}]);
  });

  test('callback exception rolls back and propagates the original error', () async {
    final database = await openTestDatabase();
    await database.execute('CREATE TABLE transaction_probe (id INTEGER PRIMARY KEY)');
  ...
  test('database closes cleanly and reopening does not rerun migrations', () async {
    final databasePath = await temporaryDatabasePath();
    final database = await openTestDatabase(databasePath: databasePath);
  ...
  test('in-memory databases do not share schema or records', () async {
    ...
    await first.execute('CREATE TABLE isolation_probe (id INTEGER PRIMARY KEY)');
    await first.insert('isolation_probe', {'id': 1});

    expect(first.path, inMemoryDatabasePath);
    expect(second.path, inMemoryDatabasePath);
    expect(
      await second.rawQuery("SELECT name FROM sqlite_master WHERE name = 'isolation_probe'"),
      isEmpty,
    );
    expect(await second.query('schema_migrations'), hasLength(1));
  });
  ...
  test('database open failure propagates to the caller', () async {
    ...
    await expectLater(
      openTestDatabase(databasePath: path.join(blockingFile.path, 'blocked.db')),
      throwsA(anything),
    );
